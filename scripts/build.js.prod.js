...
function _copydevm(suffix, done) {
  ...
  fs.readFile(`${libdir}/${name}-${suffix}.mjs`, 'utf8', (err1, data) => {
    ...
    fs.writeFile(`${dist}/lib/${name}-${suffix}.mjs`, content, { encoding: 'utf8' }, (err2) => {
      ...
      const d2 = new Date() - d1;
      process.stdout.write(`Finished '\x1b[36mcopy:es6:${suffix}\x1b[89m\x1b[0m' after \x1b[35m${d2} ms\x1b[89m\x1b[0m\n`);
      done();
    });
  });
}
...
function _copydev(suffix, done) {
  ...
  fs.readFile(`${libdir}/${name}-${suffix}.js`, 'utf8', (err1, data) => {
    ...
    fs.writeFile(`${dist}/lib/${name}-${suffix}.js`, content, { encoding: 'utf8' }, (err2) => {
      ...
      const d2 = new Date() - d1;
      process.stdout.write(`Finished '\x1b[36mcopy:umd:${suffix}\x1b[89m\x1b[0m' after \x1b[35m${d2} ms\x1b[89m\x1b[0m\n`);
      done();
    });
  });
}
...
function _doLibs(done) {
  ...
  for (let i = 0; i < keys.length; i++) {
    _copydev(keys[i], _next);
    _copydevm(keys[i], _next);
    _makeminified(keys[i], _next);
    _makeminifiedm(keys[i], _next);
  }
}
